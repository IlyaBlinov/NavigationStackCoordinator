...
final class SecondFlowCoordinatorAssembly: ISecondFlowCoordinatorAssembly {
	...
	func assemblyViewInt(_ model: ViewIntModel.SceneInput) -> ViewInt{
		ViewIntAssembly().assembly(model: model, output: SecondFlowViewIntOutput(coordinator: self.coordinator))
	}
	
	func assemblyViewDouble(_ model: ViewDoubleModel.SceneInput) -> ViewDouble  {
		ViewDoubleAssembly().assembly(model: model, output: SecondFlowViewDoubleOutput(coordinator: self.coordinator))
	}
	
	func assemblyString(_ model: ViewStringModel.SceneInput) -> ViewString  {
		ViewStringAssembly().assembly(model: model, output: SecondFlowViewStringOutput(coordinator: self.coordinator))
	}
